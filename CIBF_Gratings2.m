...
n.trials_movedir = 100;
n.movedirs = length(movedirs2);
n.trainAngs = 2;

n.trials = n.trials_movedir*n.movedirs*n.trainAngs;
...
for ii = 1:n.trainAngs
    traindirs = [traindirs; ones(n.trials/n.trainAngs,1)*ii];
end

traindirs = traindirs(randperm(n.trials));
DATA(:,D.TrainAngle_idx) = traindirs;

% Movement Directions
traindirs = [];
for ii = 1:n.movedirs
    traindirs = [traindirs; ones(n.trials_movedir,1)*ii];
end

% Designate movement directions
for ii = 1:2 
    DATA(DATA(:,D.TrainAngle_idx)==ii, D.Movement_Direction) = movedirs2(traindirs(randperm(n.trials/2)),ii);
end
...
% Convert indices to angles
DATA(:,D.TrainAngle) = angles.train(DATA(:,D.TrainAngle_idx));
DATA(:, D.TrainAngle_movedir) = angles.movedirs(DATA(:, D.TrainAngle_movedir_idx));
DATA(:, D.AdditionalAngle_movedir) = angles.movedirs(DATA(:, D.AdditionalAngle_movedir_idx));
