...
#%% First setup some parameters
    ...
    display_images = True             # Set this to true to show movies and plots
    fname = ['C:/Users/uqarento/Documents/angela_renton_local/CIBF_Gratings/2018-07-18/Exp2/results/2018-07-18_Exp2_WHOLERECORDING.tif']  # filename to be processed
    # fname = ['C:/Users/uqarento/Documents/angela_renton_local/CIBF_Gratings/2018-07-18/Exp2/results/Sue_2x_3000_40_-46.tif']  # filename to be processed

    fr = 29                            # imaging rate in frames per second
...
#%% play the movie
    ...
    # print(m_orig.shape)
    downsample_ratio = 1
    # offset_mov = -np.min(m_orig[:])
    # moviehandle = m_orig.resize(1, 1, downsample_ratio)

    # if display_images:
        # moviehandle.play(gain=10, offset=offset_mov, fr=10, magnification=2)
...
#%%% MOTION CORRECTION
    ...
    min_mov = 0.0 #cm.load(fname[0]).min()
...
#%% compare with original movie
    ...
    # display_images = True
    # if display_images:
    #     moviehandle.play(fr=30, q_max=99.5, magnification=2, offset=0)  # press q to exit
...
#%% Extract DF/F values
    ...
    F_dff = detrend_df_f(cnm2.A, cnm2.b, cnm2.C, cnm2.f, YrA=cnm2.YrA,
                         quantileMin=8, frames_window=145)

#%% Show final traces
    cnm2.view_patches(Yr, dims=dims, img=Cn)

#Trying to figure out what the fuck all these variables are.
    plt.figure()
    plt.plot(np.transpose(cnm2.C)) # This is the output trace
    plt.plot(cnm2.S[1,:])
...
#%% play along side original data
    ...
    #                axis=2)
    # if display_images:
    #         moviehandle.play(fr=30, gain=15, magnification=2, offset=0)  # press q to exit
